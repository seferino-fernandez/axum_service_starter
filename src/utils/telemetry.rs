...
pub async fn initialize_opentelemetry_providers(
    ...
    let tracer = tracer_provider.tracer(app_config.application.name.clone());
...
fn init_otel_resources(app_config: &AppConfig) -> Resource {
    Resource::builder()
        .with_service_name(app_config.application.name.clone())
        .with_attributes(vec![
            KeyValue::new(SERVICE_NAME, app_config.application.name.clone()),
            KeyValue::new(
